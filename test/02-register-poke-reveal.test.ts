...
/**
 * 파티에 들어오는 문은 **명단 한 줄마다 다른 토큰**이다 (ADR-32).
 *
 * 번호는 남도 알아서 열쇠가 못 된다 — 그래서 참가자는 번호를 치지 않고,
 * 번호가 하는 일은 **명단을 만드는 것**뿐이다. 그리고 이 문은 인증 없이 열려 있어서,
 * 제한이 없으면 "이 사람이 이 파티에 있나"를 되묻는 창구가 된다.
 */
describe("초대 명단", () => {
  /*
   * 번호를 치는 칸은 **운영자 명단 하나뿐이다** (ADR-32). 그래서 여기서 틀리면 조용하다 —
   * 그 줄에도 토큰이 생기고 링크도 나가므로 화면에 티가 안 나고, 파티 당일에야 드러난다.
   * 같은 번호가 다르게 저장되면 같은 사람이 두 줄이 되고, 토큰도 두 개가 된다.
   */
  it("★ 사람이 번호를 넣는 여러 길이 모두 같은 번호가 된다", () => {
    /*
     * 치는 것만이 아니다. 붙여넣고, 자동완성을 쓰고, 연락처에서 가져온다.
     * **어느 길로 와도 같은 번호가 저장돼야** 한 사람이 명단에 한 줄로 남는다.
     */
    for (const raw of [
      "01012345678",
      "010-1234-5678",
