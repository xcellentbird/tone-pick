...
describe("파티 룰 도움말", () => {
   ...
   *
   * 한동안 문답 둘이 그 일을 했는데(ADR-34), 바로 위 그림이 이미 `투표` 와 `파티` 로 갈라
   * 놓은 것을 글로 되풀이하는 자리였다. 카드 둘을 걷고 **그림에 조건을 건다** —
   * 투표 칸은 `콕` 을 한 글자도 말하지 않고, 파티 칸이 콕을 맡는다.
   */
  it("★ 투표 칸은 `콕` 을 쓰지 않고, 파티 칸이 콕을 맡는다", async () => {
    const step = (key: string) => HELP.steps.find((v) => v.key === key)!;
    expect(step("prevote").body, "투표 칸이 콕을 말했다").not.toContain("콕");
