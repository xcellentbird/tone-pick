...
describe("오류 화면", () => {
  ...
  it("★ 망이 끊기면 ApiError 로 감싼다 — 날 TypeError 를 올려보내지 않는다", async () => {
    /*
     * 감싸지 않으면 `userMessage` 가 없어 화면이 `ENTRY.notFound`("그런 파티가 없어요") 로
  ...
  it("★ 서버 탓을 링크 탓으로 돌리지 않는다", async () => {
     ...
     * 화면이 그때마다 "그런 파티가 없어요" 로 떨어졌다 — 500 도, 401 도, 429 도.
     *
  ...
  it("★ 회차를 잃은 실패 화면은 다른 회차로 데려가지 않는다", async () => {
    /*
     * 실제로 나온 신고다. 지워진 회차의 주소를 열면 `그런 파티가 없어요` 가 맞게 떴는데,
...
describe("파티 룰 도움말", () => {
   ...
   *
   * 한동안 문답 둘이 그 일을 했는데(ADR-34), 바로 위 그림이 이미 `프로필 투표` 와 `파티` 로 갈라
   * 놓은 것을 글로 되풀이하는 자리였다. 카드 둘을 걷고 **그림에 조건을 건다** —
   * 프로필 투표 칸은 `콕` 을 한 글자도 말하지 않고, 파티 칸이 콕을 맡는다.
   */
  it("★ 프로필 투표 칸은 `콕` 을 쓰지 않고, 파티 칸이 콕을 맡는다", async () => {
    const step = (key: string) => HELP.steps.find((v) => v.key === key)!;
    expect(step("prevote").body, "프로필 투표 칸이 콕을 말했다").not.toContain("콕");
...
describe("발표 후 참가자 탭", () => {
  ...
  /**
   * 이 한 줄은 **앱이 하지 않는 일을 가리키면 안 된다.**
   *
   * 인스타를 나누는 건 두 사람이지 앱이 아니다 (ADR-42). `인스타가 열려요` 처럼 쓰면
   * 화면 어디에도 없는 것을 찾아 헤매게 되고, 못 찾으면 그 질문이 운영자에게 간다.
   *
   * 그리고 **상대를 지목하지 않는다.** `상대가 안 열었어요` 는 이름 붙은 거절이고,
   * 서로 콕 찌른 사이라 상대가 누구인지 이미 아는 화면에서 그건 가장 아픈 문장이다.
   */
  it("★ 다음 걸음 한 줄이 앱에 없는 것을 가리키지 않는다", async () => {
    for (const [why, word] of [
      ["앱이 열어준다고 읽힌다", "열려"],
      ["앱 어딘가에 있다고 읽힌다", "확인"],
      ["상대를 지목한다", "상대가"],
      ["거절로 읽힌다", "거절"],
      ...
      expect(REVEAL.nameNote, `'${word}' — ${why}`).not.toContain(word);
    }
    // 있는 쪽을 가리킨다 — 두 사람이 직접 나누는 것
    expect(REVEAL.nameNote, "무엇을 하면 되는지 말해야 한다").toContain("인스타");
  });
 ...
 * 운영자가 초대 명단에 번호를 넣을 때 시트가 통째로 키보드 뒤로 들어갔다.
 *
...
describe("내 정보 고치기", () => {
   ...
   *
   * ⚠️ 참가자 탭의 어깨너머 가리기(`PEOPLE.cover`)는 **다른 기능이라 그대로 있다.**
   */
