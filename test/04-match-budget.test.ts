...
describe("콕 상한", () => {
  ...
  it("★ 한쪽만 찌른 것은 운영자 응답에도 숫자로만 남는다", async () => {
    /*
     * **도움말이 이걸 말하지는 않는다** (ADR-76 — 그 문답을 걷어냈다). 그래도 보장은 그대로다 —
     * 말하지 않기로 한 것이지 달라진 것이 아니라서, 지키는 자리가 여기 하나로 남는다.
     *
     * 서로 찌른 쌍은 운영자가 본다 — 발표를 누르는 게 사람이라 그럴 수밖에 없다.
     * **한쪽만 찌른 것은 다르다**: 자리 배정 재료(`oneWay`)로
