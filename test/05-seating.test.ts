...
describe("나이차", () => {
   ...
   *
   * ⚠️ **세제곱으로 바꾸면서(ADR-56) 이 숫자가 커졌다.** 절벽(10살에 +30)은 9살을 공짜로 보고
...
describe("서로 찌른 쌍", () => {
  ...
  /**
   * ★ **초반에는 새 만남이 끌림을 이기고, 후반에 갚는다** (ADR-56).
   *
...
describe("어느 라운드가 마지막이 되어도 (ADR-56)", () => {
  /**
   * ★ **상호 콕은 갚은 뒤에도 계속 끌린다.**
   *
...
describe("나이대 이성을 두루 만난다 (ADR-56)", () => {
