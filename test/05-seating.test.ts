...
describe("나이차", () => {
   ...
   *
   * ⚠️ **선형으로 바꾸면서(ADR-80) 이 숫자가 또 커졌다.** 세제곱은 3살을 0.027 로 봐서 좁은
...
describe("같은 성별 쌍은 나이차 벌점을 받지 않는다 (ADR-80)", () => {
  ...
  /**
   * ★ **한쪽만 찌른 콕은 상호가 쌓이기 전에 자리를 못 움직인다** (ADR-87).
   *
...
describe("동성 재회 벌점은 이성보다 가볍다 (ADR-81)", () => {
  ...
  it("★ 이성 재회를 만들어서라도 흩는다 — 지난 테이블의 남자 셋이 갈린다", () => {
     ...
     *
     * ⚠️ **ADR-87 이 이걸 뒤집었다.** 예전에는 동성이 0.2 라 *이성 재회와 부딪히면 지는* 쪽이었고
...
describe("서로 찌른 쌍", () => {
  ...
  /**
   * ★ **새로 만날 사람이 고갈되면 쌍이 다시 붙는다** (ADR-57 의 성질, ADR-87 이후로 더 뚜렷하다).
   *
...
describe("어느 라운드가 마지막이 되어도 (ADR-57)", () => {
  ...
  it("★ 마지막 라운드에 상호 콕 쌍이 같은 테이블에 있다", () => {
    ...
    // 확률적 규칙이라 씨앗 하나로는 못 잰다 — 평균으로 본다 (실측 1.00, ADR-87 이후)
