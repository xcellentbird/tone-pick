...
## `--seating` — 건네도 되는 가장 작은 판

```
MASTER_PIN=**** node scripts/party-export.mjs --seating --name 2026-09-1회차
```

**나이 · 성별 · 인원 · 성비 · 라운드별 테이블 수 · 라운드별 자리뿐이다.**
이름도 닉네임도 MBTI 도 매력 문구도 콕도 들어가지 않는다. 사람은 `["M", 28]` 두 값이고
자리에서는 번호(`p0`)뿐이라, **누구인지 가리키는 값이 하나도 없다.**

`--pokes` 도 필요 없다 — 끌림을 재생하지 않고 **나이 분포·성비·테이블 수가 만드는 것**을 잰다.
누가 누구를 만나게 되는가, 나이대 이성을 다 만났는가가 그 판으로 다 재어진다.
파일에 쓰면서 화면에도 그대로 뿌리므로 복사해서 건네기 쉽다.

나이차 벌점이나 `MEET_GAP`, 재회 벌점을 손볼 때 **지어낸 표본 대신 이 판으로 재라** —
`test/05-seating.test.ts` 의 24~44세 균등 난수는 실제 나이 분포와 다르고, 라운드 수에 따라
결론이 뒤집히는 것을 봤다.
