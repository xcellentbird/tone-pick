...
> ⚠️ **설문 부분은 슬라이스 27 이 바꿨다** (ADR-88). 운영자는 **누가 무엇을 골랐는지** 보고,
> 참가자는 **숫자를 보지 않으며**, 설문 **여러 개가 함께** 열린다. 화면은 현황 탭이 아니라 **설문 탭**이다.
> 아래는 그 전의 계약이다 — 텍스트 알림 부분과 첫 규칙(선택지에 사람을 넣지 않는다)은 그대로다.
> 설문 절의 *실시간 집계* · *열린 투표는 한 번에 하나* · *현황 탭에서 보낸다* 는 지금 규칙이 아니다 — `27-host-poll.md` 를 본다.
>
> **텍스트 알림은 반쯤이다 (2026-09-24 확인).** 서버(`POST /api/host/events/:id/announcements` 에 `poll` 없이)와
> 참가자 쪽 소식 줄(📢)은 있지만, **운영자가 글을 쓰는 화면이 없다** — 설문 탭은 설문만 보낸다.
> 화면을 붙일 때는 이 글의 텍스트 알림 절이 계약이다.
...
- 예약 발송. 파티 당일의 전환은 사람이 누른다 (ADR-14) — *그 근거는 ADR-93 으로 바뀌었지만(파티 시작도 예약이 연다) 알림 예약 발송은 여전히 만들지 않았다*
