...
> 구현됐고 2.5.0 으로 나갔다. 아래 표의 `지금` 칸은 **이 슬라이스 전**이다 — 지금 모양은 `이 슬라이스` 칸이다.
...
| 입장 확인창 (새로) | `.dialog.narrow` · 열릴 때 번호 칸에 커서 (ADR-63 예외 — `Sheet` 에 `autoFocus` 를 더한다) · 11자리 → 바로 `{ phone }` (답이 250ms 넘게 늦으면 `확인하고 있어요`) · `pin: "required"` 면 칸 하나, `"set"` 이면 칸 둘 · 4자리(둘이면 일치할 때) → `{ phone, pin }` · 실패 시 PIN 칸만 비움 · `left ≤ 2` 부터 남은 횟수 · 잠기면 PIN 칸 비활성, 번호 칸은 산다 |
...
| `ENTRY.pinWrong(left)` | `PIN 번호가 맞지 않아요` / `PIN 번호가 맞지 않아요 — ${left}번 더 틀리면 잠겨요` (left ≤ 2) |
...
- **옛 `test/13-personal-link.test.ts`** — **뒤집힌다.** S-A1(*토큰 없으면 못 연다*)이 정반대가 됐다.
  이 파일은 걷어내고 15 로 옮긴다. 남길 것은 *등록 폼이 번호를 받지 않는다*(S-A3 → 15 의 S-B4) 뿐이다
  (걷어냈다 — 13 의 시나리오 문서도 함께 지웠고, 그 설계와 근거는 ADR-32·75 에 있다)
