/**
 * 스테이지 워커의 하루 상한 (슬라이스 35 S-B2 → 슬라이스 37, ADR-97 후기 4 · ADR-99).
 *
 * 스테이지 워커에는 로그인이 없다 — 한국에서 주소를 아는 사람은 누구나 연다. 그래도 **프로덕션의 하루 한도는 지킨다.**
 ...
 *
 * **QA 를 부른 횟수로 센다.** 처음엔 스테이지 만들기와 명령을 따로 셌는데(후기 4), 스테이지가 100명까지 커지고
 ...
 *    (12명 스테이지 = QA 33번 · DO 53번). `DAILY` 를 다 써도 4,800번 남짓, 5% 다
 *  - 쓴 줄 — 추적에 줄 수가 안 나와 **코드에서 셌다.** 등록 한 명이 6~7줄(참가자 · 토큰 · PIN 과 색인), 명단 한 줄.
 *    스테이지 만들기에 몫을 다 쓰는 날이 가장 나쁘다 — 몫 하나에 3.5줄쯤, `DAILY` 면 1만 줄 남짓, 10% 다
 * 100명 스테이지 하나가 230쯤 드니 하루에 열두 번 남짓이다 (ADR-99). `DAILY` 를 올리려면 이 셈부터 다시 한다.
 *
 * 하루는 **UTC** 로 끊는다. Cloudflare 의 하루 한도가 00:00 UTC(한국 오전 9시)에 다시 찬다.
 */
export const DAILY = 3_000;

/** 요청 하나마다 이 워커가 스스로 쓰는 몫 — 로비에 몫을 받고 돌려주는 왕복과 스테이지 DO 하나 */
...
/** 스테이지 하나를 만드는 데 드는 어림 — 로그인·회차·명단, 한 명에 두 번, 파티까지 가는 걸음, 요청마다의 몫 */
...
export const tooBig = (need: number, left: number): string =>
  `이 스테이지는 QA 호출이 ${need.toLocaleString("ko-KR")}번쯤 필요한데 오늘은 ${left.toLocaleString("ko-KR")}번 남았어요. 인원을 줄이거나 오전 9시 이후에 다시 만들어주세요.`;
