 ...
 *
 * **잰 결과 (2026-09-24, ADR-97 후기)** — 한국에서 두 길 다 200, `label: "QA"`. 한국 밖은 못 쟀다.
 *
 * ⚠️ **이 워커를 프록시로 만들지 마라.** 경로나 본문을 받아 QA 로 넘기는 순간, 바인딩 요청에 `cf` 가
 * 없다면 QA 국가 문의 우회로가 된다. 부르는 경로는 코드에 박힌 것뿐이어야 한다.
 */
interface Env {
  APP: Fetcher;
}
