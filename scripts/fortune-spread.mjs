 ...
 *
 * ⚠️ **`--var` 로 넣은 값은 다음 자동 배포에 덮인다.** Cloudflare 의 git 연동이 브랜치에
 *    푸시가 있을 때마다 QA 를 다시 배포하고, 그때는 `wrangler.jsonc` 의 vars 만 실린다.
 *    스윕 중에 아무 PR 이나 머지되면 **온도가 조용히 기본값으로 돌아가고**,
 *    `--temp 1.2` 라고 이름 붙인 판이 실제로는 기본값 판이 된다 —
 *    `--temp` 는 받아 적는 이름표라 이걸 막지 못한다.
 *
 *    **스윕은 한자리에서 끝낸다.** 도는 동안 머지하지 마라.
 *    중간에 머지가 끼었다면 그 뒤 판은 버리고 다시 뽑는다.
 *    (오래 걸리는 스윕이라면 `wrangler.jsonc` 의 qa vars 에 값을 적고 머지하는 편이 안전하다 —
 *     자동 배포가 그 값을 싣는다. 다 끝나면 되돌린다.)
 *
