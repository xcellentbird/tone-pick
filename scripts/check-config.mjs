 ...
 *  6. **스테이지 워커가 QA 아닌 곳을 겨누는 것** (슬라이스 35 S-A3) — 공개된 워커라 프로덕션을
...
// ⑥ 스테이지 워커의 표적은 QA 하나다 (슬라이스 35 S-A2·S-A3, ADR-97)
//
...
if (!app) {
  problems.push("스테이지 워커(scripts/qa/worker)에 APP 바인딩이 없습니다");
} else if (!QA_NAME || app.service !== QA_NAME) {
  problems.push(`스테이지 워커의 APP 바인딩이 "${app.service}" 를 가리킵니다. QA("${QA_NAME}")만 됩니다 (ADR-97)`);
}
if ((STAGE.services ?? []).length !== 1) {
  problems.push(`스테이지 워커의 서비스 바인딩은 APP 하나여야 합니다 — 지금: ${stageTargets.join(", ") || "없음"}`);
}
if (STAGE.env) {
  problems.push("스테이지 워커에 env 가 있습니다. 환경마다 표적이 갈릴 자리를 두지 않습니다 (S-A2)");
}
// 스테이지 워커가 로그인하는 PIN 은 QA 의 공개 PIN 그대로여야 한다 — 어긋나면 스테이지를 하나도 못 만든다
if (STAGE.vars?.QA_PIN !== qa?.vars?.MASTER_PIN) {
  problems.push(`스테이지 워커의 QA_PIN("${STAGE.vars?.QA_PIN}")이 QA 공통 PIN("${qa?.vars?.MASTER_PIN}")과 다릅니다`);
}
// 사람에게 보여 줄 주소도 QA 여야 한다. **점까지 본다** — `tone-pick-qa-tool.` 도 앞 글자가 같다
if (!String(STAGE.vars?.QA_PUBLIC_URL ?? "").startsWith(`https://${QA_NAME}.`)) {
  problems.push(`스테이지 워커의 QA_PUBLIC_URL("${STAGE.vars?.QA_PUBLIC_URL}")이 QA(${QA_NAME}) 주소가 아닙니다 — 참가 링크가 엉뚱한 곳을 엽니다`);
}
// 스테이지 워커에는 로그인이 없다 (ADR-97 후기 4). 국가 문은 QA 와 **같은 값**이어야 한다 (S-B3) —
...
if (!STAGE.vars || !("ALLOWED_COUNTRIES" in STAGE.vars)) {
  problems.push("스테이지 워커 vars 에 ALLOWED_COUNTRIES 키가 없습니다. 도구만 해외에 조용히 열립니다 (S-B3). 끄려면 지우지 말고 비우세요");
} else if (STAGE.vars.ALLOWED_COUNTRIES !== qa?.vars?.ALLOWED_COUNTRIES) {
  problems.push(`스테이지 워커의 ALLOWED_COUNTRIES("${STAGE.vars.ALLOWED_COUNTRIES}")가 QA("${qa?.vars?.ALLOWED_COUNTRIES}")와 다릅니다 — 둘은 같이 움직입니다 (S-B3)`);
}
