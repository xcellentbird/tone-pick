 ...
 *
 * 2026-09-05 기준 실측 200.8 KiB (js 154.5 · 로고 41.5 · css 5.5 · html 4.9 · 아이콘 3.1).
 *
 * **입장 확인창이 첫 화면에 얹히면서 199.6 → 200.8 로 넘었다** (ADR-75 — 번호 + PIN 번호 칸이
 * 참가 링크를 연 사람이 곧바로 지나는 길이라 첫 청크에 있어야 한다). 넘긴 건 1.2 KiB 지만
 * 8월 28일의 185 KiB 에서 여기까지 온 것은 버전 표시·문구·화면이 조금씩 쌓인 결과다 —
 * 그 사이 눈금이 한 번도 안 울렸으니 이 줄이 처음으로 "알고 넘기는" 자리다.
 *
 * 처음 값(2026-08-28)은 185.0 KiB — **로고가 들어오면서 143 → 185 로 올랐다.** 세는 범위에
 * 이미지를 더한 것과 같은 변경이라, 이 줄이 diff 에 남는 것이 그 장치의 목적이다
