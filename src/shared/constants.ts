 ...
 *
 * **서버에 묻지 않는다.** 배포는 워커와 정적 자산이 함께 나가지만 브라우저는 옛
 * `index.html` 을 들고 있을 수 있다 — 그때 서버에서 받아 온 숫자를 띄우면
 * **지난 버전 화면이 새 버전이라고 말한다.** 버전을 확인하는 순간이 하필 그 화면이
 * 이상할 때라, 그 거짓말은 정확히 답이 필요한 자리에서 난다.
 * 빌드에 박힌 값이면 옛 화면은 옛 숫자를 말한다 — 그게 곧 그 버그의 답이다.
 *
...
export const SEAT_W = {
  /**
   * 나이차 벌점의 계수. `min(나이차,10)/10` 에 곱한다 — **한 살에 0.1 씩 곧게** (ADR-83).
   * **이성 쌍에만 건다** (ADR-80). 동성끼리는 나이가 자리를 가르지 않는다.
   */
  AGE: 1.0,
  /**
   * 이미 만난 사이. **만난 횟수에 그대로 비례한다** (ADR-91) — 상한이 없다.
   * 두 번이면 2.0, 세 번이면 3.0. 다양성이 가장 앞이라 가장 세게 자라는 항이다.
   * 예전에는 `min(1, 횟수/2)` 로 1.0 에서 멈췄다. **이성 쌍** (ADR-80).
   */
  REP: 1.0,
  /**
   * 같은 성별의 재회. **이성의 절반**이다.
   *
   * 0 으로 뒀더니(ADR-80) 같은 사람들이 라운드마다 다시 앉았다 — 이성 쪽 사정이 그들을
   * 가르지 않으면 흩을 이유 자체가 없었기 때문이다.
   *
   * ⚠️ **0.5 는 ADR-81 이 그은 선(0.3)을 넘은 값이다** (ADR-91). 그 선은 *동성을 흩느라
   * 이성 재회를 만들지 마라* 는 뜻이었고, 지금은 **이성 재회도 같이 피하는 쪽**으로 바뀌었다 —
   * 재회 벌점 전체가 상한 없이 자라서 어느 쪽이든 두 번째 만남이 비싸다.
   ...
   *
   * 5.0 인 것은 재회 벌점이 상한 없이 자라기 때문이다 (ADR-91) — 2.5 면 세 번 만난 쌍이
   * 벌점에 밀려 흩어진다. 그래도 **8 이상으로 올리지 마라** (ADR-57).
   */
  MUTUAL: 5.0,
  /**
   * 처음 만나는 나이대 이성. **줄지 않는다** (ADR-91) — 끝까지 1.0 이다.
   * 진행도에 따라 0.4 까지 줄던 때는 후반에 재회 벌점이 이 항을 눌러서,
   * *아직 안 만난 나이대 이성* 이 아니라 **아무나 안 만난 사람**으로 자리가 흘렀다.
   */
  NEW_BASE: 1.0,
  NEW_SPAN: 0.0,
  /**
   * 콕은 진행도에 **비례한다** (ADR-91) — `POKE_SPAN × 진행도`.
   * 상호가 쌓이기 전에는 0 이라, **한쪽만 찌른 콕은 초반에 자리를 못 움직인다.**
   * 만나보고 고른 신호라 무겁게 보되, 그 무게는 파티가 진행돼야 생긴다.
   */
  POKE_BASE: 0.0,
  POKE_SPAN: 1.5,
  /**
   * 떼어 놓을 쌍이 같은 테이블일 때의 벌점 (ADR-90). **가중치가 아니라 규칙이다** —
   * 한 사람이 한 테이블에서 얻을 수 있는 값을 다 합쳐도(스무 명 테이블에서 200 남짓) 못 넘게 크게 둔다.
   *
   * 인원으로 나누지 않는다. 나누면 큰 테이블일수록 가벼워져, 새 만남 점수가 규칙을 이긴다.
   * 무한대로 두지 않는 이유는 **다 지킬 수 없는 판**이다 — 그때도 어기는 쌍이 가장 적은 배치를 골라야 한다.
   */
  APART: 1000,
} as const;
 ...
 *
 * 벌점에는 문턱이 없다(선형은 연속이다). 이 값은 *누구를 만나게 할 것인가* 의 기준이라
 * 성격이 다르다 — 나이차가 이보다 큰 쌍은 새로 만나도 점수를 주지 않는다.
 *
 * **6 에서 8 로 넓혔다** (ADR-91). 7~8살 차이 이성은 벌점이 0.7~0.8 로 가벼운데도
 * 이어줄 이유가 0 이라 끝내 안 만나졌다 — 벌점이 막던 게 아니라 **목표에 없던 것**이다.
 * 넓히면 첫 배정이 오히려 촘촘해진다: 이어줄 상대가 많아져 먼 쌍을 고를 필요가 없다.
 */
export const MEET_GAP = 8;
...
/**
 * **등록에서 받는 나이** (ADR-87). 서버·화면·문구가 이 하나를 본다.
 *
 * 상한이 99 이던 때는 장난으로 91 을 적을 수 있었고, 그 한 사람이 자리 배정의 목적함수를
 * 통째로 끌고 갔다 (ADR-78 이 벌점에 상한을 씌워 막았다). 48 은 그 장난을 **애초에 안 받는다** —
 * 벌점 상한이 뒤에서 받쳐주고 이 값이 앞에서 거른다.
 *
 * ⚠️ **이미 등록한 사람은 건드리지 않는다.** 검사는 들어오는 값에만 걸리므로 49세 이상으로
 * 등록된 옛 참가자는 그대로 남는다 — 파티 도중에 누가 명단에서 사라지면 안 된다.
 */
export const AGE_RANGE = { min: 18, max: 48 } as const;
 ...
 *
 * **참가자 쪽과 달리 영구 잠금이 아니다.** 참가자의 5회 잠금은 운영자가 푸는데(ADR-75),
 * 운영자 위에는 푸는 사람이 없다 — 영구로 만들면 운영자가 자기 앱에서 영영 잠긴다.
 * 그래서 창이 지나면 저절로 풀린다. 파티 당일에 손이 떨려 다섯 번 틀린 사람이
 * 10분을 기다리는 것이 상한이다.
 *
 * 5회 / 10분은 하루 720번이다 — 네 자리라면 만 가지를 다 대보는 데 열나흘, 여섯 자리면 몇 년이다.
 *
 * ⚠️ **접속지를 바꾸면 이 제한은 넘어간다.** 그렇다고 회차 전체로 세면 아무나 운영자를
 * 잠글 수 있다 — 그게 더 나쁘다. 여기서 막는 것은 한 자리에서 두드리는 일이다.
 */
export const HOST_PIN_TRIES = { max: 5, windowMs: 10 * 60_000, warnAt: 2 } as const;
