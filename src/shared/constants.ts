...
export const SEAT_W = {
   ...
   */
  AGE: 1.0,
  /** 이미 만난 사이. `min(1, 만난 횟수/2)` 에 곱한다. **이성 쌍** (ADR-80) */
  REP: 0.6,
  /**
   * 같은 성별의 재회. 이성의 **1/3** 이다 (ADR-81).
   *
   * 0 으로 뒀더니(ADR-80) 같은 사람들이 라운드마다 다시 앉았다 — 이성 쪽 사정이 그들을
   * 가르지 않으면 흩을 이유 자체가 없었기 때문이다. **크기가 규칙의 전부다**: 이성 재회를
   * 피하는 일과 부딪히면 지고, 이성 쪽이 아무 말도 하지 않는 자리에서만 동성을 가른다.
   *
   * **0.3 으로 올리지 마라** — 거기서부터 남자 셋을 굳이 흩어 이성 재회를 만든다
   * (`test/05-seating.test.ts` 의 `이성 재회와 부딪히면 진다`). 올려서 얻는 것도 적다:
   * 동성 재회는 값이 아니라 판이 정해서, 이성과 같은 값까지 올려도 3%p 남짓이다.
   */
  REP_SAME: 0.2,
