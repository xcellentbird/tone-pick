...
export const SEAT_W = {
  ...
  AGE: 1.0,
  /**
   * 이미 만난 사이. **만난 횟수에 그대로 비례한다** (ADR-87) — 상한이 없다.
   ...
   *
   * 5.0 인 것은 재회 벌점이 상한 없이 자라기 때문이다 (ADR-87) — 2.5 면 세 번 만난 쌍이
   * 벌점에 밀려 흩어진다. 그래도 **8 이상으로 올리지 마라** (ADR-57).
   */
  MUTUAL: 5.0,
  /**
   * 처음 만나는 나이대 이성. **줄지 않는다** (ADR-87) — 끝까지 1.0 이다.
  ...
  NEW_SPAN: 0.0,
  /**
   * 콕은 진행도에 **비례한다** (ADR-87) — `POKE_SPAN × 진행도`.
 ...
 *
 * **6 에서 8 로 넓혔다** (ADR-87). 7~8살 차이 이성은 벌점이 0.7~0.8 로 가벼운데도
