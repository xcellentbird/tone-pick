 ...
 *
 * 넘게 두면 한 쌍이 나이차를 통째로 밀어내고 그 테이블만 이상해진다 —
 * 마지막 라운드에서 이미 겪었다 (ADR-11). 서로 좋아하는데 나이차가 큰 쌍은
 * **마지막 라운드에서** 붙인다. 거기서는 가중치가 아니라 구조가 붙여준다.
 */
 ...
 *
 * 참가자 입장 칸과 운영자 명단 칸이 **같은 값을 쓴다** — 두 곳이 갈라지면
 * 같은 번호가 다르게 저장되고, 그건 파티 당일 문 앞에서야 드러난다.
 ...
 * 참가자는 문 앞에서 막히고, 운영자 명단에서는 아무도 못 들어오는 번호가 조용히 들어간다.
