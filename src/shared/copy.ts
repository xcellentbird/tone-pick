...
export const PEOPLE = {
  ...
  mine: "나",
  /**
   * 목록 머리에서 **아래 사람들**을 두고 하는 말이다. 그래서 "볼 수 있어요" 다 —
   * 내 카드 밑에 있을 때는 반대 방향(남들이 나를)이라 주어를 따로 박아야 했지만,
   * 자리가 목록 위로 오면서 주어가 자리로 정해졌다.
   *
   * **감춰진 게 있을 때만** 붙는다. 파티가 시작되면 감출 게 없으니 이 줄도 사라진다 (ADR-21).
   * 이 줄이 없으면 카드에 빈 자리만 보여서 "왜 없지" 가 된다.
   */
  agesAtParty: "파티가 시작되면 나이·MBTI 를 볼 수 있어요.",
