...
export const PEOPLE = {
  ...
  mine: "나",
  /**
   * 목록 머리 줄의 **왼쪽**. 오른쪽 남은 콕과 **한 줄에 들어가야 해서 짧다** —
   * `파티가 시작되면 나이·MBTI 를 볼 수 있어요.` 는 좁은 폰에서 두 줄로 접혔다.
   * 늘리려거든 오른쪽과 합쳐 360px 폭에 들어가는지 먼저 본다.
   *
   * 주어는 **자리가 정한다.** 목록 위라서 아래 사람들 이야기이고, 그래서 주어가 없어도 된다 —
   * 내 카드 밑으로 옮기면 반대 방향(남들이 나를)이 되어 "남들에게" 를 박아야 한다.
   *
   * **감춰진 게 있을 때만** 붙는다. 파티가 시작되면 감출 게 없으니 사라진다 (ADR-21).
   * 이 줄이 없으면 카드에 빈 자리만 보여서 "왜 없지" 가 된다.
   */
  agesAtParty: "나이·MBTI 는 파티부터 보여요.",
  /**
