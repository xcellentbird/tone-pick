...
export const PEOPLE = {
  ...
  mine: "나",
  /**
   * **감춰진 게 있을 때만** 붙는다. 파티가 시작되면 감출 게 없으니 이 줄도 사라진다.
   *
   * "다른 사람에게는 이렇게 보여요" 는 뺐다 — `mine` 태그와 맨 위라는 자리가 이미 말한다.
   * 대신 **"남들에게"** 를 넣는다. 앞 문장이 없으면 주어가 흐려져서
   * *내가* 남의 나이를 보게 된다는 뜻으로도 읽힌다.
   *
   * 이 줄이 없으면 "등록할 때 분명히 냈는데 왜 없지" 가 된다 (ADR-21).
   */
  mineLater: "나이·MBTI 는 파티가 시작돼야 남들에게 보여요.",
