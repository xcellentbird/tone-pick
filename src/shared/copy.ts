...
export const PEOPLE = {
  ...
  /**
   * 내 카드 **오른쪽 칸**의 라벨. 남들 카드의 👉 자리에 같은 폭으로 들어간다 —
   * 그 칸이 "내가 콕을 쓰는 자리" 라서, 내 줄에서는 얼마나 남았는지를 말한다.
...
export const FORTUNE = {
  ...
  prompt: {
    system:
      ...
      "  위 문단들의 결과 이어지게 쓰되, 반드시 아래를 지킵니다.\n" +
    ...
    /**
     * 사주의 재료만 보낸다 — 이름·생년월일·성별. **이름은 답변에 나오지 않는다** (위 규칙).
     * 닉네임·나이·MBTI·매력은 여기 없다. 그건 어필 호출의 재료다.
     */
    user: (v: {
      realName: string;
      gender: "M" | "F";
...
/**
 * 오늘의 어필 — 오늘 탭의 **두 번째 카드**. 운세 아래에 선다.
 *
 * 운세가 "오늘 나는 어떤 결인가" 라면 이건 **"그래서 뭘 어떻게 보여주나"** 다.
 * 그래서 운세 결과를 재료로 받는다 (`appealInput`) — 두 카드가 서로를 모르면
 * 한 화면에 남남인 글 두 개가 된다.
 *
 * **이름은 여기 오지 않는다.** 실명 예외는 운세 호출 하나뿐이다 (ADR-20 개정).
 */
export const APPEAL = {
  title: "오늘의 어필",
  prompt: {
    system:
      "당신은 오늘 파티에 가는 사람에게 '내 매력을 어떻게 보여줄까' 를 짚어 주는 사람입니다.\n\n" +
      "오늘 자리는 이렇습니다.\n" +
      ...
      "연기하라고 하지 마세요. 이미 그 사람이 가진 것을 **어떤 자리에서 어떻게 꺼내면 되는지**만 말합니다.\n\n" +
      '반드시 JSON 만 출력하세요: {"headline": "...", "tips": ["...", "...", "..."]}\n' +
      "headline: 25자 이내 한 줄. 오늘 이 사람의 어필을 한마디로.\n" +
      "tips: 받은 매력과 **같은 개수, 같은 순서**로. 매력 하나에 팁 하나입니다.\n" +
      "  각 팁은 70자 이내 한 문장.\n" +
      "  · **언제·누구에게·어떻게** 가 들어가야 합니다. '자신감을 가지세요' 는 팁이 아닙니다\n" +
      "  · 그 매력이 실제로 드러나는 **장면 하나**를 잡아 주세요\n" +
      "  · 오늘의 운세와 미션이 함께 주어집니다. 결이 어긋나지 않게 쓰되 미션을 그대로 반복하지 마세요",
    /** **이름은 없다.** 나이·성별·매력과 방금 나온 운세만 넘긴다 */
    user: (v: {
      age: number;
      gender: "M" | "F";
      charms: string[];
      ...
      `${v.age}세 · ${v.gender === "M" ? "남성" : "여성"}\n` +
      `본인이 쓴 매력 (이 순서대로 팁을 주세요)\n` +
      v.charms.map((c, i) => `  ${i + 1}. ${c}`).join("\n") +
      `\n\n오늘의 운세 (참고)\n${v.fortune.headline}\n${v.fortune.body}` +
      `\n\n오늘의 미션 (참고)\n${v.fortune.mission}`,
  },
  /** LLM 이 없어도 이 카드는 떠야 한다. 운세만 뜨고 여기가 비면 화면이 반쯤 죽어 보인다 */
  fallback: {
    headline: [
      "가진 걸 그대로 꺼내는 밤",
      "애쓰지 않아도 되는 자리예요",
    ...
    tip: (charm: string, variant: number) =>
      [
        `'${charm}' 이야기가 나오면 한 발만 더 들어가 보세요. 짧게 덧붙이는 정도면 충분해요.`,
        `'${charm}' 은 말로 꺼내기보다 자리에서 자연히 드러나요. 그 순간이 오면 피하지 마세요.`,
        `대화가 잠깐 멎으면 '${charm}' 쪽으로 이야기를 돌려보세요. 물어보는 형태면 더 편해요.`,
      ][variant % 3],
  },
} as const;
