/**
 * 자리 배정. 설계와 실측치는 `docs/SEATING.md`, 왜 이 모양인지는 ADR-57.
 *
 ...
 *
 * **방향이 있다** (ADR-57) — 내가 보낸 것만 나에게 만족을 준다. 콕은 익명이라
...
export function buildSeating(input: BuildInput): Seat[] {
  ...
  /*
   * **재시작을 여러 번 한다** (ADR-57). 예산을 늘려도 결과가 같았다는 실측이 있었는데,
...
class World {
  ...
  constructor(input: BuildInput) {
    ...
    /*
     * **진행도.** 라운드 번호가 아니라 **쌓인 상호 콕**으로 잰다 (ADR-57) —
    ...
    for (let i = 0; i < n; i++) {
      ...
      for (let j = 0; j < n; j++) {
         ...
         *
         * **재회는 만난 횟수에 그대로 비례한다** (ADR-87) — 상한이 없다. 두 번이면 2.0,
 ...
 *
 * ⚠️ `give` 는 인원으로 나누고 `fresh` 는 나누지 않는다 (ADR-57). 나누면 이성이 3명인
