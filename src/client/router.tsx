...
export const router = createBrowserRouter([
  ...
  // 참가자
  /*
   * 참가 링크는 **회차 아이디 하나**다 (ADR-75). 회차마다 하나라 단톡방에 뿌려도 된다.
   * 열쇠는 전화번호 + PIN 번호다 — ADR-32 가 토큰으로 막던 것(번호를 아는 사람이 그 사람이 되는 것)을
   * PIN 번호가 막는다. 입장 코드는 담지 않는다 (ADR-13).
   * 입장 확인창도 라우트다 — 뒤로 가기로 닫힌다.
   */
  { path: "/j/:id", element: plain(<Join />) },
  { path: "/j/:id/enter", element: plain(<Join />) },
